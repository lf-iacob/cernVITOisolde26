...
void spectrum_comp(const vector<string> &files, TString output = "comparison/output.root") {
  ...
  cout<<endl;
  vector<Color_t> colours={kPink-8, kViolet-3, kAzure+2, kTeal+3, kOrange-3, kSpring-5, kTeal+2, kYellow+1, kBlack};
  ...
  */
  ...
  for (size_t i=0; i<nFiles; i++) {
    ...
    legc->AddEntry(hc[i], Form("%s - %s - Trigger Level = %s mV", s[i].c_str(), rads[i].c_str(), tl[i].c_str()));
  ...
  for (size_t i=0; i<nFiles; i++) {
    ...
    lega->AddEntry(hamp[i], Form("%s - %s - Trigger Level = %s mV", s[i].c_str(), rads[i].c_str(), tl[i].c_str()));
